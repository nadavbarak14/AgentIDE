...
- **FR-001**: System MUST discover extensions by fetching a generated `extensions/index.json` that lists all available extensions at application load time. The index is generated by the build tooling (Vite plugin) at dev-server start and build time by scanning the `extensions/` directory for subfolders containing a `manifest.json` file.
