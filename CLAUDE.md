...
- TypeScript 5.7, Node.js 20 LTS + Express 4, commander (CLI), better-sqlite3, node-pty, ws 8, React 18, Vite 6, Tailwind CSS 3 (017-readme-npm-publish)
- N/A — no schema changes (014-remote-preview-proxy)
- TypeScript 5.7, Node.js 20 LTS + `@playwright/test` (new dev dependency), existing release test helpers (019-e2e-release-tests)
- N/A — tests use ephemeral temp directories with SQLite (via the real server) (019-e2e-release-tests)
- TypeScript 5.7, Node.js 20 LTS + React 18, Vite 6, Tailwind CSS 3, Express 4, better-sqlite3 (all existing) + `react-resizable-panels` (new), `@dnd-kit/core` + `@dnd-kit/sortable` (new) (020-flexible-panel-layout)
- SQLite (better-sqlite3) — one migration: `ALTER TABLE panel_states ADD COLUMN layout_config TEXT DEFAULT NULL` (020-flexible-panel-layout)
...
## Recent Changes
- 020-flexible-panel-layout: Added TypeScript 5.7, Node.js 20 LTS + React 18, Vite 6, Tailwind CSS 3, Express 4, better-sqlite3 (all existing) + `react-resizable-panels` (new), `@dnd-kit/core` + `@dnd-kit/sortable` (new)
- 019-e2e-release-tests: Added TypeScript 5.7, Node.js 20 LTS + `@playwright/test` (new dev dependency), existing release test helpers
